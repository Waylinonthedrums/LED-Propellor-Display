...
// display one verticale bar to test timing (only needed without magnetic sensor)
...
int main() {
    ...
    while(true){ 
        ...
        //Code to use without magnetic sensor to synchronize revolutions,
        // times are estimates with my equipment setup, lengths are not linear
        ...
        for(int i = 0; i < 50; i++) {
            ...
            wait(0.020f); // wait for 30 chars - 0.012f?
        }
        wait(0.5f);
        for(int i = 0; i < 30; i++) {
        ...
        wait(0.5f);
        for(int i = 0; i < 30; i++) {
            string_loop("PERSISTENCE OF VISION         ", 30);
            wait(0.012f); // wait for 30 chars
        }
        wait(0.5f);
        for(int i = 0; i < 30; i++) {
        ...
        //wait(0.075f); // wait for 15 chars
        
        //string_loop("EMBEDDED S", 10);
        //wait(0.1f); // wait for 10 chars
        
        //string_loop("EMBED", 5); 
        //wait(0.12659f); // wait for 5 chars
        
        //char_loop("A", 1);
        //wait(0.141f); // for once character
        
        //test_bar_lights();
        //wait(0.168f); // for vertical bar
